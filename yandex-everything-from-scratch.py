...
Original file is located at
    https://colab.research.google.com/drive/1z19n5u0mx_S4-DrjxG3jdsT0-HhJPzy6
...
class Sequential(Module):
    ...
    def backward(self, input, gradOutput):
        ...
        Make sure that the input for `i-th` layer is the output of `module[i-1]`
        ...
        for i in reversed(range(len(self.modules))):
            grad = self.modules[i].backward(inputs[i], grad)
        self.gradInput = grad
        return self.gradInput
...
class SoftMax(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.divide(exp_x, sum_exp)
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        for i in range(batch_size):
            ...
            outer_prod = np.outer(softmax_output[i], softmax_output[i])
            jacobian = diag_softmax - outer_prod
...
class LogSoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = gradOutput - self.softmax * sum_gradOutput
...
class BatchNormalization(Module):
    EPS = 1e-3 # как я понял обязательно
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        dx_norm = gradOutput
        dvar = np.sum(
            dx_norm * self.x_centered * (-0.5) * self.std_inv ** 3,
            axis=0
        )

        dmean = np.sum(dx_norm * (-self.std_inv), axis=0)

        #i hate it
        self.gradInput = (
                dx_norm * self.std_inv
                + dvar * 2.0 * self.x_centered / batch_size
                + dmean / batch_size
        )
...
class LeakyReLU(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        # Your code goes here. #############################################
        self.gradInput = np.multiply(gradOutput, np.where(input > 0, 1.0, self.slope))
        return self.gradInput
...
class ELU(Module):
    ...
    def updateOutput(self, input):
        ...
        mask = input <= 0
        self.output[mask] = self.alpha * (np.exp(input[mask]) - 1)
        return self.output

    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        self.gradInput = np.copy(gradOutput)
        mask = input <= 0
        self.gradInput[mask] = np.multiply(gradOutput[mask], self.alpha * np.exp(input[mask]))
        return self.gradInput
...
class SoftPlus(Module):
    ...
    def updateOutput(self, input):
        ...
        mask_large = input > 20
        self.output[mask_large] = input[mask_large] + np.log(1 + np.exp(-input[mask_large]))

        mask_small = ~mask_large
        self.output[mask_small] = np.log(1 + np.exp(input[mask_small]))

        return self.output

    def updateGradInput(self, input, gradOutput):
        # Your code goes here. ################################################
        sigmoid_x = 1 / (1 + np.exp(-input))
        self.gradInput = np.multiply(gradOutput, sigmoid_x)
        return self.gradInput
...
class ClassNLLCriterionUnstable(Criterion):
    ...
    def updateOutput(self, input, target):
        ...
        batch_indices = np.arange(input.shape[0])

        # Compute the negative log-likelihood loss
        self.output = -np.log(input_clamp[batch_indices, target_indices]).mean()
        return self.output
    ...
    def updateGradInput(self, input, target):
        ...
        # Gradient: -target / input_clamp, averaged over batch
        ...
        if len(target.shape) == 1 or (len(target.shape) == 2 and target.shape[1] == 1):
            ...
            for i in range(batch_size):
                if len(target.shape) == 1:
                    ...
                    idx = target[i, 0]
                self.output -= input[i, idx]
            self.output /= batch_size
        else:
            # Target is one-hot encoded
            batch_size = input.shape[0]
            product = np.multiply(target, input)
            self.output = -np.sum(product) / batch_size

        return self.output

    def updateGradInput(self, input, target):
        # Your code goes here. ################################################
        # The gradient of negative log-likelihood: dL/dx = -target
        batch_size = input.shape[0]

        # Initializinng gradInput with zeros
        self.gradInput = np.zeros_like(input)

        # Check if target is one-hot encods
        if len(target.shape) == 1 or (len(target.shape) == 2 and target.shape[1] == 1):
            # Target contains class indices
            for i in range(batch_size):
                if len(target.shape) == 1:
                    ...
                    idx = target[i, 0]
                self.gradInput[i, idx] = -1.0 / batch_size
        else:
            # Target is one-hot encoded
...
class Conv2d(Module):
    ...
    def updateOutput(self, input):
        ...
        if input.ndim == 4:
            ...
            for b in range(batch_size):
                for out_c in range(self.out_channels):
                    for in_c in range(self.in_channels):
                        self.output[b, out_c] += sp.signal.correlate2d(
                            padded_input[b, in_c],
                            self.W[out_c, in_c],
                            mode='valid'
                        )
            ...
            for out_c in range(self.out_channels):
                for in_c in range(self.in_channels):
                    self.output[out_c] += sp.signal.correlate2d(
                        padded_input[in_c],
                        self.W[out_c, in_c],
                        mode='valid'
                    )
                self.output[out_c] += self.b[out_c]
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        if input.ndim == 4:
            ...
            for b in range(batch_size):
                for in_c in range(self.in_channels):
                    for out_c in range(self.out_channels):
                        flipped_kernel = np.flip(self.W[out_c, in_c], axis=(0, 1))
                        padded_grad = np.pad(gradOutput[b, out_c], ((pad_size, pad_size), (pad_size, pad_size)),
                                             mode='constant')
                        self.gradInput[b, in_c] += sp.signal.correlate2d(
                            padded_grad,
                            flipped_kernel,
                            mode='valid'
                        )
            ...
            for in_c in range(self.in_channels):
                for out_c in range(self.out_channels):
                    flipped_kernel = np.flip(self.W[out_c, in_c], axis=(0, 1))
                    padded_grad = np.pad(gradOutput[out_c], ((pad_size, pad_size), (pad_size, pad_size)),
                                         mode='constant')
                    self.gradInput[in_c] += sp.signal.correlate2d(
                        padded_grad,
                        flipped_kernel,
                        mode='valid'
                    )
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        if input.ndim == 4:
            ...
            for b in range(batch_size):
                for out_c in range(self.out_channels):
                    self.gradb[out_c] += np.sum(gradOutput[b, out_c])

                    for in_c in range(self.in_channels):
                        padded_input = np.pad(input[b, in_c], ((pad_size, pad_size), (pad_size, pad_size)),
                                              mode='constant')
                        self.gradW[out_c, in_c] += sp.signal.correlate2d(
                            padded_input,
                            gradOutput[b, out_c],
                            mode='valid'
                        )
            ...
            for out_c in range(self.out_channels):
                ...
                for in_c in range(self.in_channels):
                    padded_input = np.pad(input[in_c], ((pad_size, pad_size), (pad_size, pad_size)), mode='constant')
...
class MaxPool2d(Module):
    ...
    def updateOutput(self, input):
        ...
        pooled = input_reshaped.max(axis=(3, 5))

        self.max_indices = np.zeros((batch_size, channels, output_h, output_w, 2), dtype=np.int64)

        for b in range(batch_size):
            for c in range(channels):
                for i in range(output_h):
                    for j in range(output_w):
                        window = input_reshaped[b, c, i, :, j, :]
                        #получаем линейный индекс
                        linear_idx = np.argmax(window)
                        # переводим в 2д индекс
                        h_idx, w_idx = np.unravel_index(linear_idx, (self.kernel_size, self.kernel_size))
                        self.max_indices[b, c, i, j] = [h_idx, w_idx]


        if batch_size == 1 and input.shape[0] == 1:
            self.output = pooled[0]
        else:
            self.output = pooled
    ...
    def updateGradInput(self, input, gradOutput):
        if input.ndim == 4:
            ...
            batch_size = 1
            input = input.reshape(1, *input.shape)
            gradOutput = gradOutput.reshape(1, *gradOutput.shape)
        ...
        for b in range(batch_size):
            for c in range(channels):
                for i in range(output_h):
                    for j in range(output_w):
                        h_offset, w_offset = self.max_indices[b, c, i, j]

                        h_pos = i * self.kernel_size + h_offset
                        w_pos = j * self.kernel_size + w_offset

                        self.gradInput[b, c, h_pos, w_pos] = gradOutput[b, c, i, j]
